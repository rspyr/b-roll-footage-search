...
import { Loader2, ArrowLeft, Clock, FileVideo, HardDrive, AlertCircle } from "lucide-react";
...
export default function VideoDetail() {
  ...
  const { data: video, isLoading, error } = useGetVideo(id, { query: { enabled: !!id } });
  
  const frameScrollRef = useRef<HTMLDivElement>(null);
  const transcriptionScrollRef = useRef<HTMLDivElement>(null);

  // Scroll to timestamp if provided
  useEffect(() => {
    if (timestamp && video && !isLoading) {
      const ts = parseFloat(timestamp);
      
      // We would ideally scroll to the specific elements
      // Simplistic approach for now
      setTimeout(() => {
        const frameEl = document.getElementById(`frame-${ts}`);
        if (frameEl) frameEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
        
        const transEl = document.getElementById(`trans-${ts}`);
        if (transEl) transEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }, 500);
    }
  }, [timestamp, video, isLoading]);
                ...
                {video.frames.map(frame => (
                  <div 
                    ...
                    className={`flex flex-col sm:flex-row gap-4 rounded-md p-2 transition-colors
                      ${timestamp && Math.abs(parseFloat(timestamp) - frame.timestampSec) < 1 ? 'bg-primary/10 ring-1 ring-primary/30' : 'hover:bg-accent/50'}`}
                  >
                ...
                {video.transcriptions.map(trans => (
                  <div 
                    ...
                    className={`rounded-md p-3 border border-border/50 text-sm transition-colors
                      ${timestamp && parseFloat(timestamp) >= trans.startSec && parseFloat(timestamp) <= trans.endSec ? 'bg-primary/10 border-primary/30' : 'bg-background hover:bg-accent/50'}`}
                  >
